          ...
          <div class="modal-container">
            <div class="modal-body">
              <div
                class="loader"
                v-show="!isLoad"
                style="margin-top:18px;width: 64px;height: 64px;border: 9px dotted #110f69;border-top: 9px dotted #110f69;border-right: 9px dotted #110f69;border-bottom: 9px dotted #110f69;"
              ></div>
          ...
          <div class="card">
            <div class="card-header" style="padding: 10px;">
              ...
              <button
                ...
                @click="createOrder"
              >
                New Bill <i class="fa fa-plus-circle"></i>
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="card">
            <div class="card-header" style="padding: 10px;">
              <downloadExcel
                ...
                name="filename.xls"
                style="margin:10px;"
                ><i class="fa fa-download"></i>&nbsp;&nbsp;Back Up
                Data</downloadExcel
              >
              <button
                type="button"
            ...
            <table class="table table-striped">
              <thead>
                <tr style="color: rgb(17, 15, 105);font-weight: 800;">
                  ...
                  <th style="padding-bottom: 26px;">
                    Product Name<br /><input
                      type="text"
                      class="form-control"
                      id="usr"
                      v-model="search_name"
                  ...
                  <th style="padding-bottom: 27px;">
                    ...
                    <select
                      ...
                      v-model="search_cat"
                      @change="getFilterItem"
                    >
                      <option value=""></option>
                      <option
                        v-for="value in categories.category"
                        :key="value"
                        :value="value"
                        >{{
                  ...
                  <th>
                    Order Date<br /><input
                      style="width: 91%;padding: 2px 6px;height: 25px;margin-top: 2px;"
                      type="date"
                      class="form-control"
                      id="usr"
                      v-model="from_date"
                      @change="getFilterItem"
                    /><br /><input
                      style="width: 91%;margin-top: -16px;padding: 2px 6px;height: 25px;"
                      type="date"
                      class="form-control"
                      id="usr"
                      v-model="to_date"
                  ...
                  <th
                    style="padding-bottom: 64px;text-align:center;"
                    colspan="2"
                  >
                    Actions
                  </th>
              ...
              <tbody>
                <tr
                  v-if="
                    filterBy(
                      filterBy(itemOrders, search_name, 'pro_name', 'comment'),
                      search_cat,
                      'category'
                    ).length <= 0
                  "
                ...
                >
                  ...
                  <td align="center">
                    <a
                      @click="openimage(item.file)"
                      v-if="
                        item.imageType == 'image/png' ||
                          item.imageType == 'image/gif' ||
                          item.imageType == 'image/jpeg' ||
                          item.imageType == 'image/jpg' ||
                          item.imageType == 'image/bmp'
                      "
                      data-toggle="modal"
                      data-target="#myModal"
                      style="font-size: 2.5em;color: #110e69;cursor: pointer;"
                      ><i class="fa fa-camera"></i
                    ></a>
                    <a
                      :href="item.file"
                      v-else-if="
                        item.imageType == 'application/pdf' ||
                          item.imageType == 'text/plain'
                      "
                      target="_blank"
                      style="font-size: 2.5em;color: #110e69;cursor: pointer;"
                    ...
                    <a
                      :href="item.file"
                      v-else-if="
                        item.imageType == 'application/vnd.ms-excel' ||
                          item.imageType == 'image/tiff' ||
                          item.imageType ==
                        ...
                        <div class="modal-content">
                          <div class="modal-body">
                            <div
                              class="loader"
                              v-show="!isLoad"
                              style="margin-top:18px;width: 64px;height: 64px;margin-left: 1%;border: 9px dotted #110f69;border-top: 9px dotted #110f69;border-right: 9px dotted #110f69;border-bottom: 9px dotted #110f69;"
                            ></div>
                            <img
                              :src="imageurl"
                              height="100%"
...
export default {
  ...
  methods: {
      ...
      docRef
        ...
        .then(function(querySnapshot) {
          querySnapshot.forEach(function(doc) {
            ...
            ref.getMetadata().then(function(metadata) {
              thisI.json_data.push(
                Object.assign(doc.data(), { imagename: metadata.fullPath })
              );
              thisI.showloder = false;
            });
          });
    ...
    newCategory: function() {
      window.open(
        this.$router.resolve({ name: "addnewcategory" }).href,
        "_blank"
      );
    },
    getFilterItem() {
      if (this.search_cat) {
        this.itemOrders = this.items.filter(
          m => m.category === this.search_cat
        );
      } else if (this.from_date && this.to_date) {
        this.itemOrders = this.itemOrders.filter(
          m => m.order_date >= this.from_date && m.order_date <= this.to_date
        );
      } else {
        this.itemOrders = this.items;
      }
