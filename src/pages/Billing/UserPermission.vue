        ...
        <div class="modal-wrapper">
          <div class="modal-container">
            ...
            <div class="modal-body" style="margin: 0;">
              <div v-if="category_user" class="alert alert-danger" role="alert">
                {{ category_user }}
              </div>
              <div class="modal-body" style="text-align: left;">
                ...
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-sm-12">
                    <p>
                      Is Admin? :
                      <input type="checkbox" v-model="select_level" />
                    </p>
                  </div>
                </div>
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-sm-12">
                    <p>
                      ...
                      >
                        <option
                          v-for="value in filterBy(categories.category)"
                          :key="value"
                          :value="value"
                          >{{
        ...
        <div class="card">
          <div class="card-header" style="padding: 10px;">
            ...
            <button
              ...
              @click="addUserCategory('add')"
            >
              New User<i class="fa fa-plus-circle"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="col-md-12">
        <div class="card">
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-if="filterBy(user_data).length <= 0">
              ...
              <tr
                v-for="(value, index) in limitBy(user_data, 10, page * 10 - 10)"
                :key="index"
              >
                <td>{{ index + 1 + page * 10 - 10 }}</td>
                <td>
                  <i class="fa fa-user" v-if="value.is_admin === true"></i>
                  {{ value.user_name }}
                </td>
                <td>
                  <span
                    id="assign_category"
                    :key="index"
                    v-for="(category, index) in value.category_permission"
                  >
                    <span
                      ><i
                        class="fa fa-times cat-list"
                        @click="removeCategory(value.id, category)"
                      ></i
                      >&nbsp;&nbsp;{{ category }}</span
                    >
                  </span>
                ...
                <td>
                  <div v-if="value.is_admin !== true">
                    <a
                      @click="deleteUserCategory(value.id)"
                ...
                <td>
                  <a
                    ...
                    "
                    style="font-size: 2.5em;color: rgb(11, 122, 34);cursor: pointer;"
  ...
  methods: {
    ...
    removeCategory: function(id, category) {
      ...
      docRef
        .doc(id)
        .update({
          category_permission: firebase.firestore.FieldValue.arrayRemove(
            category
          )
        })
        .then(function() {
